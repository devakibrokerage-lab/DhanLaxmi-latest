        ...
        <div className="open-order-bottom-window fixed bottom-0 left-0 right-0 z-50 bg-[var(--bg-card)] border-t border-[var(--border-color)] shadow-2xl p-4 transition-transform duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-4 flex justify-between items-end">
                ...
                <div className="text-right">
                    <span className={`text-xl font-bold px-2 py-0.5 rounded ${pnlChipBg} ${pnlColor}`}>{money(netPnl)}</span>
                    <p className="text-xs text-[var(--text-muted)] mt-1">Net P&L (after entry brokerage)</p>
                </div>
            </div>
            ...
            <div className="mb-4 p-2 bg-[var(--bg-secondary)] rounded-lg">
                ...
                <DetailRow label="Order Instant" value={productType} colorClass="text-indigo-500" />
                <DetailRow label="Expire Date" value={formattedStockExpireDate} colorClass="text-[var(--text-muted)]" />
                {(stop_loss !== 0 && stop_loss != null) && <DetailRow label="Stop Loss" value={stop_loss} colorClass="text-[var(--text-muted)]" />}
                {(target !== 0 && target != null) && <DetailRow label="Target" value={target} colorClass="text-[var(--text-muted)]" />}
            ...
            <div className="p-3 bg-[var(--bg-secondary)] rounded-lg mb-4">
                ...
                <div className="space-y-3">
                    <div className="flex items-center space-x-2">
                        ...
                        />
                        <div className="text-xs text-[var(--text-muted)] italic">Size: <span className="font-medium text-[var(--text-primary)] ml-1">{lotSize}</span></div>
                    </div>
            ...
            {(userRole === 'broker' || isOpen) && (
                <div className="flex space-x-2">
                    ...
                    {userRole === 'broker' && <button
                        ...
                        disabled={submitting}
                        className={`flex-1 p-3 rounded-lg text-[var(--text-primary)] font-semibold transition bg-[var(--bg-secondary)] border border-[var(--border-color)] hover:bg-[var(--bg-hover)] ${submitting && action === 'Close' ? 'opacity-50' : ''}`}
                    >
