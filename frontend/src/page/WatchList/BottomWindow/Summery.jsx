...
function Summery({
  ...
  return (
    <div className="p-2 h-full">
      ...
      <div className="flex space-x-4 mb-6">
        <button className={`flex-1 p-2 rounded-lg text-xs font-semibold transition ${getProductTypeClass('Intraday')}`} onClick={() => setProductType('Intraday')}>INTRADAY</button>
        <button className={`flex-1 p-2 rounded-lg text-xs font-semibold transition ${getProductTypeClass('Overnight')}`} onClick={() => setProductType('Overnight')}>OVERNIGHT</button>
      </div>
      ...
      {feedback && (
        <div className={`p-2 mb-3 rounded-md text-sm ${feedback.type === 'error' ? 'bg-red-500/20 text-red-500' : 'bg-green-500/20 text-green-500'}`}>
          {feedback.message}
        </div>
      )}
      ...
      <div className="p-3 bg-[var(--bg-secondary)] rounded-lg mb-4">
        {/* <h4 className="text-lg font-semibold mb-5 text-[var(--text-primary)]">{actionTab === 'Buy' ? 'Place Buy Order' : 'Place Sell Order'}</h4> */}
        ...
        <div className="space-y-3">
          ...
          <div className="relative mt-4">
            ...
            />
            <label className="absolute left-3 -top-2.5 bg-[var(--bg-card)] px-1 text-xs  peer-focus:text-indigo-500 peer-focus:scale-100 peer-placeholder-shown:scale-100 transition-all leading-none">
              Lot <span className="text-[12px]">(Lot Size: {lotSize})</span>
            </label>
          </div>
          ...
          {userRole === 'broker' && (
            <div className="relative mt-5 ">
              <input
                ...
                className="peer w-full p-3 bg-transparent text-[var(--text-primary)] rounded-md border border-[var(--border-color)] outline-none focus:border-indigo-500 transition-colors"
              />
              <label className="absolute left-3 -top-2.5 bg-[var(--bg-card)] px-1 text-xs  peer-focus:text-indigo-500 transition-all leading-none flex items-center gap-1">
                Jobbing %
              </label>
          ...
          <div className="text-sm bg-[var(--bg-input)] rounded-md p-4 flex flex-col border border-[var(--border-color)] mt-6 mb-6 gap-2">
            {userRole === 'broker' && (
          ...
          <div className="flex space-x-2">
            ...
            {(userRole === 'broker' || isOpen) && (
              <button
                ...
                disabled={submitting}
                className={`flex-1 p-4 rounded-lg text-white font-bold text-lg shadow-md transition-transform active:scale-95 ${actionTab === 'Buy' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'} ${submitting ? 'opacity-50' : ''}`}
              >
